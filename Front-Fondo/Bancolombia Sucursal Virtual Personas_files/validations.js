...
function noContieneCadena(value,cadena){
...
function checkMinNumberAndLetter(value){
	return ( /\d/.test(value) && /[a-zñ]/i.test(value));
}
...
function checkNumberKey(numberField, e) {
    checkKey (numberField, e, /\d+/);
}
function checkDashedNumberKey(numberField, e) {
    checkKey (numberField, e, /(\d+-*)*/);
}

function checkKey(field, e, re) {
    var currentLen = field.value.length;
    var whichCode = (window.Event) ? e.which : e.keyCode;
    if (currentLen == 0 || whichCode == 8) 
        return;
    var tempField = "" ;
    var matches =  field.value.match(re);
    if (matches) 
        tempField = matches[0];
    field.value = tempField;
}

function leftZeroTrim(value) {
    var result;
    value = trim(value);
    return  value != "" && (result = value.match(/[1-9]\d*/)) != null? result.toString(): value;
}

function TieneSoloDigitosyPunto(Cadena){
	var Digitos = "0123456789.";
	var TodosValidos = true;
	for (i = 0;  i < Cadena.length;  i++){
		ch = Cadena.charAt(i);
		for (j = 0;  j < Digitos.length;  j++)
			if (ch == Digitos.charAt(j))	break;
			if (j == Digitos.length){
				TodosValidos = false;
				break;
			}
...
function TieneSoloDigitos(Cadena){
...
function noTieneSoloDigitos(Cadena){
...
function checkSelect (theField, mens){
	if(theField.selectedIndex == 0){
		alert("Se debe seleccionar " + mens)
		theField.focus()
		return false
	}
	else {return true}
	...
	function LeapYear(intYear) {
   	...
   	function fechaValida(month,day,year) {
		var err = 0;
		var intday;
		var intMonth;
		var intYear
...
function trim(valueToTrim){
	return valueToTrim.replace(/^\s+|\s+$/g,'');
}

/*
* Allow alphabetic characters also "," "." and white space. Not numbers and not special characters.
*/
...
/*
* not white space. Not numbers and not special characters.
*/
function validateConceptDataNoSpace(e) { 
    tecla = (document.all) ? e.keyCode : e.which; 
     if (tecla==8) return true; 
    if (tecla==0) return true;
    if (tecla==95 || tecla==32){
    	if(isIE){
			window.event.keyCode=0;}
    	return false; 
    }
...
/*
* Allow white space. Not numbers and not special characters.
*/
function validateConceptDataNoSpecial(e) { 
    tecla = (document.all) ? e.keyCode : e.which; 
    if (tecla==8) return true; 
    if (tecla==0) return true;
    if (tecla==95){
